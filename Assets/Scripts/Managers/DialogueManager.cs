...
namespace MeatGame.Dialogue
{
	public class DialogueManager : MonoBehaviour
	{
		...
		*/
		...
		private TextAsset activeDialogueTextAsset;
		...
		public void StartDialogue(int _dialogueFileID, int _dialogueID)
		{
			...
			PlayerMenu.Instance.TryCheckMenuFreeze();
			...
			if (SaveData.Instance.ViewedDialogue.Contains(dialogueFullIDString) == false) // Add this dialogue to the list of viewed dialogues
			{
				SaveData.Instance.ViewedDialogue.Add(dialogueFullIDString);
		...
		public void ParseSentence(string sentence) // Responsible for interpreting text files
		{
			...
			else if (newBranch == true)
			{
				...
				if (sentence.IndexOf("OPTIONCONDITIONS") > 0)
				{
					...
					bool conditionsTrue = ConditionChecker.Instance.CheckConditions(isolatedConditions);
			...
			else if (sentence.StartsWith("BACKGROUND_MUSIC =")) // Sets the background music
			{
				...
				else
				{
					...
					AudioManager.Instance.PlayMusic(clipA);
				}
				AdvanceDialogue();
			}
			...
			else if (sentence.StartsWith("REALTIME"))
			{
				...
				TimeManager.Instance.SetRealTimeActive(isRealtime);
				AdvanceDialogue();
			}
			else if (sentence.StartsWith("ADDTIME")) // e.g ADDTIME(Hour,2)
			{
				...
				TimeManager.Instance.AddTime(unit, amount);
				CheckCooldowns();
				AdvanceDialogue();
			}
			else if (sentence.StartsWith("SETTIME")) // e.g SETTIME(1430) for 2:30
			{
				...
				TimeManager.Instance.SetTime(timeString);
				CheckCooldowns();
				AdvanceDialogue();
			}
			...
			else if (sentence.StartsWith("REMOVEPOSSESSION")) // e.g REMOVEPOSSESSION(bones,1)
			{
				...
				sentence = sentence.Substring(0, sentence.IndexOf(")"));
				int.TryParse(sentence, out quantity);
				InventoryManager.Instance.RemovePossession(identifier, quantity); // MeatGame.Inventory
				AdvanceDialogue();
			}
			...
			else if (sentence.StartsWith("CONDITIONBRANCH")) // e.g CONDITIONBRANCH((DOW == Friday),B)
			{
				...
				if (ConditionChecker.Instance.CheckConditions(conditions) == true)
				{
					AdvanceDialogue();
				}
				else
		...
		public void EndDialogue()
		{
			...
			TimeManager.Instance.SetRealTimeActive(false);
			PlayerMenu.Instance.TryCheckMenuFreeze();
			...
			if (dialogueTextWhole.Contains("ROOM = "))
			{
				...
				cooldownID = (roomName + "|" + TimeManager.Instance.day.ToString() + (TimeManager.Instance.hour + 3).ToString("00") + TimeManager.Instance.minute.ToString("00"));
			...
			if (dialogueTextWhole.Contains("CHARACTER = "))
			{
				...
				foreach (string line in lines) // for every line of dialogue
				{
					...
					if (line.StartsWith("CHARACTER = "))
					{
						...
						characterName = characterName.Replace("	", string.Empty);
						cooldownID = (characterName + "|" + TimeManager.Instance.day.ToString() + (TimeManager.Instance.hour + 3).ToString("00") + TimeManager.Instance.minute.ToString("00"));
						for (int i = cooldownList.Count - 1; i >= 0; i--)
						{
		...
		{
			string currentTimeID = (TimeManager.Instance.day.ToString() + TimeManager.Instance.hour.ToString("00") + TimeManager.Instance.minute.ToString("00"));
		NextCooldown:
		...
		{
			if (SaveData.Instance.ViewedDialogue.Contains(dialogueFileID.ToString() + "-" + dialogueID.ToString() + "-" + x) == false) // Add this dialogue branch to the list of viewed dialogues
			{
				SaveData.Instance.ViewedDialogue.Add(dialogueFileID.ToString() + "-" + dialogueID.ToString() + "-" + x);
		...
		private IEnumerator _Enter3DRoom(string room, string doorTransform, string startTransition = "Fadeblack_Start", string endTransition = "Fadeblack_End")
		{
			...
			EventManager.Instance.UpdateDialogueEvents();
			Transition_Animator.Play(endTransition);
			yield return null;
		}
